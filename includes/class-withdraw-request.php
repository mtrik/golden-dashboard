...
class GDB_Withdraw_Request {
        ...
        try {
            ...
            $result = $wpdb->insert(
                ...
                [
                    ...
                    'reference_id'     => 0,
                    'description'      => sprintf(
                        __('درخواست برداشت به مبلغ %s (کارمزد: %s - مبلغ قابل واریز: %s) - کد پیگیری: %s', 'golden-dashboard'),
    ...
    public static function approve($request_id, $payment_data = []) {
        ...
        $result = $wpdb->update(
            ...
            [
                ...
                'description'          => sprintf(
                    __('برداشت تایید شده به مبلغ %s (کارمزد: %s - مبلغ واریزی: %s) - کد پیگیری: %s', 'golden-dashboard'),
                    gdb_price_plain($amount),
        ...
        try {
            ...
            $wpdb->update(
                ...
                [
                    ...
                    'description'=> sprintf(__('درخواست برداشت به مبلغ %s رد شد و به کیف پول بازگردانده شد - کد پیگیری: %s', 'golden-dashboard'), gdb_price_plain($amount), $request->tracking_code),
                    'updated_at' => current_time('mysql'),
                ],
                ['id' => $request_id],
                ['%s', '%s', '%s', '%s'],
    ...
    private static function generate_tracking_code() {
        return sprintf('%012d', mt_rand(0, 999999999999));
    }
    ...
    private static function notify_admin($user_id, $amount, $tracking_code, $request_id) {
        ...
        $message = sprintf(
            __('کاربر %s درخواست برداشت به مبلغ %s ثبت کرد. کد پیگیری: %s
